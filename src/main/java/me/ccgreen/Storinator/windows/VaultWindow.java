...
public class VaultWindow {
  ...
  VaultWindow(Player play, int page) {
    player = play;
    this.page = page;
    data = StorinatorMain.playMan.getData(play);
  ...
  private void createButtons() {
    ItemStack barrier = new ItemStack(Material.IRON_BARS, 1);
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (i == 0 || player.hasPermission("Storinator.vault." + i)) {
        button = new ItemStack(Material.BLACK_STAINED_GLASS_PANE, 1);
  ...
  private ItemStack activeButton() {
    ...
    ItemStack button = new ItemStack(Material.WHITE_STAINED_GLASS_PANE, 1);
    ItemMeta meta = button.getItemMeta();
